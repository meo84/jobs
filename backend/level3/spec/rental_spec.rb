...
RSpec.describe Rental do
  ...
  describe 'creating rentals and associations' do
    describe '#initialize' do
      it 'raises an error if end_date is earlier than start_date' do
        expect { Rental.new(1, 1, "2015-12-10", "2015-12-8", 100) }.to raise_error ArgumentError, 'end_date must be later than start_date'
      end
      it 'raises an error if car_id doesn\`t exist' do
        expect { Rental.new(1, 2, "2015-12-8", "2015-12-8", 100) }.to raise_error ArgumentError, 'car_id must refer to the id of an existing car'
      end
    end

    describe '#car' do
      it 'returns car used for the rental' do
        rental = Rental.new(1, 1, "2017-12-8", "2017-12-8", 100)
  ...
  describe 'distributing commission fees' do
    ...
    before(:all) { @rental = Rental.new(1, 1, "2015-12-8", "2015-12-8", 100) }
